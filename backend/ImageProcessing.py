...
def colourQuantisation(img, n_colors):
    n_colours = n_colors
...
def create_pattern_grid(labels, image_shape, dmc_matches):
    grid = labels.reshape(image_shape[0], image_shape[1])
    pattern = np.empty(grid.shape, dtype=object)

    for i in range(image_shape[0]):
        for j in range(image_shape[1]):
            cluster_id = grid[i][j]
            dmc_code = dmc_matches.iloc[cluster_id]['DMC_COLOR']
            pattern[i][j] = dmc_code
...
def complete_pipeline(path, max_dim=50, n_colors = 15):
    ...
    colors, labels = colourQuantisation(pixels, n_colors)
    dmc_matches = find_dmc_match(colors)
    dmc_matches = stitch_counts(labels, dmc_matches)
    pattern = create_pattern_grid(labels, pixels.shape, dmc_matches)

    # building the output 
    key = {}
    for i, row in dmc_matches.iterrows():
        key[row['DMC_COLOR']] = {
            'name' : row['COLOR_NAME'],
            'rgb': list(row['rgb_tuple']),
            'count': row['stitch_count']
        }
    return {
        'pattern_grid': pattern.tolist(),
        'key' : key,
        'dimensions' : {'width': pixels.shape[1], 'height': pixels.shape[0]}
    }


# result = complete_pipeline("test3.jpeg")
# print(result)
